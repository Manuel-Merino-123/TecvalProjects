    ...
    try:
        ...
        if num_Circles > 0:
            ...
            if length > 0:
                ...
                x_offset = x0 + perp_x
                y_offset = y0 + perp_y
                center_offset_x = 0 + perp_x
                center_offset_y = 0 + perp_y
                
                # Dibujar línea de cota desplazada (azul)
                ax.plot([center_offset_x, x_offset], [center_offset_y, y_offset], 
                        color='blue', linestyle='-', linewidth=0.8)
                
                # Dibujar líneas de extensión (amarillas punteadas)
                ax.plot([0, center_offset_x], [0, center_offset_y], 
                        color='yellow', linestyle=':', linewidth=0.5)
                ax.plot([x0, x_offset], [y0, y_offset], 
                        color='yellow', linestyle=':', linewidth=0.5)
                
                # Flechas de cota (usando el mismo desplazamiento)
                ax.annotate('', xy=(x_offset, y_offset), 
                          xytext=(center_offset_x, center_offset_y),
                          arrowprops=dict(arrowstyle='<->', color='yellow', lw=1))
                
                # Texto de la cota
                midpoint_x = (center_offset_x + x_offset)/2 + perp_x * text_offset/length
                midpoint_y = (center_offset_y + y_offset)/2 + perp_y * text_offset/length
